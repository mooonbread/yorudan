...
The background rotation uses five photographs in the user’s requested order: green hills in Alaska (`forest-storm.jpg`), Pedra da Gávea in Rio de Janeiro (`tropical-rock-towers.jpg`), Lone Peak with orange tree trunks (`granite-and-pines.jpg`, unfiltered), the beach in Kauai (`sunlit-beach.jpg`), then dark summit rocks with orange lichen (`lichen-rocks.jpg`). It loops back to Alaska. The mountain panorama, green mountain pinnacles, rocky turquoise shoreline, coastal overlook, summit boulders, mossy forest stream, granite cliffs, black-and-white summit silhouettes, and the woodland trail are set aside and remain saved for reference. The user confirmed that the black-and-white summit silhouettes were not taken by them; the photographer is not yet identified. Web-ready JPEG copies live in `assets/nature/`. The granite-and-pines slide uses the user’s unfiltered version. Full-resolution PNG originals are saved in the parent workspace under `photos/nature/`.
